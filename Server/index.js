...
getWebsiteData = async (url) => {
  try {
    ...
    keywords &&
      keywords?.forEach((item) => {
        if (item?.trim()) {
          ...
          seoData?.keywordConsistency?.push({ [item]: count });
        }
      });
    ...
    seoData.canonicalLink = $("link[rel='canonical']")?.attr("href");
    //#endregion
    ...
    try {
      ...
      seoData.robotsText = data ? true : false;
    } catch (error) {
      seoData.robotsText = error ? false : true;
    }
    req.on("error", (err) => {
      console.error("Error occurred:", err.message);
    });
    ...
    seoData.numberofResources = { images, html, script, css, totalResource };
    //#endregion
...
app.get("/", async (req, res) => {
  try {
    const response = await getWebsiteData(req.query?.url);
    res.send({ status: true, response }).status(200);
  } catch (error) {
    res.send({ status: false, message: error }).status(404);
  }
});
